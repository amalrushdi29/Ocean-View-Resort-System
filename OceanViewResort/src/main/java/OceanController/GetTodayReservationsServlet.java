...
/**
 * Servlet to get today's check-ins or check-outs
 */
@WebServlet("/GetTodayReservations")
public class GetTodayReservationsServlet extends HttpServlet {
    ...
    public GetTodayReservationsServlet() {
        super();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        String type = request.getParameter("type"); // "checkin" or "checkout"
        
        JSONObject jsonResponse = new JSONObject();
        JSONArray reservationsArray = new JSONArray();
        
        try {
            // Get today's reservations from DAO
            List<ReservationBean> reservationList = reservationDao.getTodayReservations(type);
            ...
            if (reservationsArray.length() > 0) {
                ...
                jsonResponse.put("message", "No reservations for today");
            }
            
        } catch (Exception e) {
            e.printStackTrace();
            jsonResponse.put("success", false);
            jsonResponse.put("message", "Error retrieving today's reservations: " + e.getMessage());
        }
